...
size_t finder(const std::vector<T>& vector, const C& value, int start, int end){	if(vector.empty())
		return 0;
	if(vector[0] > value)
		return 0;

	int i = (start+end)/2;
	if(value > vector[i] && value < vector[i+1])
		return i+1;
	if(value == vector[i])
		return i;
	if(vector[vector.size()-1] < value)
		return vector.size();
	if(vector[vector.size()-1] == value)
		return vector.size()-1;
	if(value < vector[i])
		return finder(vector, value, start, i);
		return finder(vector, value, i+1, end);
...
size_t insertion_idx(const std::vector< T >& elements, const C& val)
{
	...
	return finder(elements, val, 0, size-1);
}
